  ...
  return (
    <div>
      ...
      <div className="table-wrap">
        <table>
          <thead>
            <tr><th>#</th><th>Member</th><th>Mobile</th><th>Donations</th><th className="text-right">{sortLabel}</th><th>Last Donation</th></tr>
          </thead>
          <tbody>
            {loading && <tr><td colSpan={6} className="text-center p-6 text-text-muted">Loading…</td></tr>}
            {!loading && rows.length === 0 && <tr><td colSpan={6} className="text-center p-6 text-text-muted">No data</td></tr>}
            {rows.map((r, i) => (
              <tr key={r.member_id}>
                <td>
                  <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${
                    i < 3 ? "bg-saffron-100 text-saffron-700" : "bg-surface-4 text-text-muted"
                  }`}>{i + 1}</span>
                </td>
